...
class NeuralNetwork(ABC):
    ...
    def forward(self, X: np.ndarray) -> np.ndarray:
        ...
        # Iterate through the network's layers.
        # we iterate and we do the forward for each layer of our 
        #design matrix and return it in the end after forwarding through
        #all the layers
        out = X
    ...
    def backward(self, target: np.ndarray, out: np.ndarray) -> float:
        ...
        #here we get the loss and the dLdY values using our target and out
        #data values
        loss = self.loss.forward(target, out)
        dLdY = self.loss.backward(target, out)
        # Backpropagate through the network's layers.
        #using the hint we will dot eh abckwrad of the dldy value and then
        #we will return the loss in the end
