...
def svd_lfm(R):
    ...
    U, sigma, V = scipy.linalg.svd(R, full_matrices=False)

    # Construct user and movie representations
    user_vecs = np.dot(U, np.diag(sigma))
    movie_vecs = V.T
...
def get_train_mse(R, user_vecs, movie_vecs):
    ...
    #we want to adjust he loss as we iterate through the matrix
    mse_loss = 0
    for i in range(R.shape[0]):
        for j in range(R.shape[1]):
            #we check if the elemnt is not there and if it is we will subtract it from the
            #dot using the formula, and using the same formula derived earlier add this to our 
            #running loss total
            if not np.isnan(R[i, j]):
...
def update_user_vecs(user_vecs, movie_vecs, R, user_rated_idxs):
    ...
    #we iterate through the matrix and use the user and movie feature vector
    #to update our faeture vectors accordingly then we will updaet our user vector
    #according to the user rated indicies
    for i in range(R.shape[0]):
        UV = np.eye(user_vecs.shape[1])
        MV = np.zeros_like(movie_vecs[0])
        for j in user_rated_idxs[i]:
            UV += np.outer(movie_vecs[j], movie_vecs[j])
            MV += R[i, j] * movie_vecs[j]
        user_vecs[i] = np.dot(np.linalg.inv(UV), MV)
...
def update_movie_vecs(user_vecs, movie_vecs, R, movie_rated_idxs):
    ...
    for i in range(R.shape[1]):
        UV = np.eye(user_vecs.shape[1])
        MV = np.zeros_like(user_vecs[0])
        for j in movie_rated_idxs[i]:
            UV += np.outer(user_vecs[j], user_vecs[j])
            MV += R[j, i] * user_vecs[j]
        movie_vecs[i] = np.dot(np.linalg.inv(UV), MV)
