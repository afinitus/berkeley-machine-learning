...
def main():
    ...
    d, ind = nbrs.kneighbors(us_features)
    near_countries = []
    for i in ind[0]:
        near_countries.append(countries[i])
    near_countries = near_countries[1:]
    print(near_countries)
...
def _rmse_grid_search(training_features, training_labels, pipeline, parameters, technique):
    ...
    sc = sklearn.metrics.make_scorer(mean_squared_error, squared=False, greater_is_better=False)
    grid = GridSearchCV(pipeline, parameters, scoring=sc)
